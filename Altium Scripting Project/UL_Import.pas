...
//    pad.Mode := ePadMode_LocalStack; // ePadMode_Simple, ePadMode_ExternalStack
    ...
    // eRoundHole; eSquareHole, eSlotHole
    If  GetBetween(Data, '(Slotted ', ')') = 'True' Then Begin
        pad.HoleType := eSlotHole;
        pad.HoleSize := MilsToCoord(Evaluate(GetBetween(Data, '(SlotHeight ', ')')));
        pad.HoleWidth := MilsToCoord(Evaluate(GetBetween(Data, '(SlotWidth ', ')')));
    End Else Begin
...
Begin
    ...
    If InitLibDocs(SavePath, Proj, ProjDoc, SchLibDoc, sLib) = False Then Begin
